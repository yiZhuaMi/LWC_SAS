...
public class STeacherInfo extends JFrame {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					STeacherInfo frame = new STeacherInfo(Info);
	 ...
	 * Create the frame.
	 */
	public STeacherInfo(Vector<Infomation> Info) {
		...
		for(int i = 0;i<count;i++)
		{
			...
			result[i][1]=Info.get(i).acdamic;
			result[i][2]=Info.get(i).department;
		}
		for(int i = 0;i<count;i++)
		{
		...
		returnbutton.addActionListener(new ActionListener() {
			...
			{
				STeacherInfo.this.setVisible(false);
			}
		});
	}
